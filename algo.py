...
class FLMBase(trainer_base.TrainerBase):
    ...
    def __init__(self, config, tokenizer):
        ...
        self.noise_time_not_data = getattr(config.algo, 'noise_time_not_data', False)
...
class FLM(FLMBase):
    def loss(self, x0, output_tokens, conditioning_tokens=None,
        ...
        if self.diffusion_forcing and conditioning_tokens is not None and self.noise_time_not_data == False:
        ...
        if self.noise_time_not_data and self.diffusion_forcing and conditioning_tokens is not None:
            df_mask = self._sample_clean_mask(conditioning_tokens)
