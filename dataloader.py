...
def _data_seed(config):
    """Seed for DATA generation / eval-set construction.

    Decoupled from the training `config.seed` (which seeds model init + batch
    order via `seed_everything`) so a multi-seed run that varies `seed` keeps an
    identical generated pool + eval set across seeds. Falls back to `config.seed`
    when `data_seed` is unset, preserving prior behavior.
    """
    ds = getattr(config, "data_seed", None)
    return config.seed if ds is None else ds
...
def _build_sudoku_splits(config):
    ...
    data_seed = _data_seed(config)
    key = (num_train, num_valid, difficulty, data_seed, num_workers)
    ...
    if _disk_cache_complete(cache_dir):
        ...
        LOGGER.info(
            ...
            "data_seed=%d) -> caching to %s",
            num_train, num_valid, difficulty, data_seed, cache_dir)
...
def _build_nqueens_splits(config):
    ...
    data_seed = _data_seed(config)
    key = (n, num_train, num_valid, data_seed)
    if key in _NQUEENS_SPLIT_CACHE:
        return _NQUEENS_SPLIT_CACHE[key]
    LOGGER.info(
        "Generating N-Queens dataset (n=%d, train=%d, valid=%d, data_seed=%d)",
        n, num_train, num_valid, data_seed)
    splits = generate_nqueens_dataset(
        n=n, num_train=num_train, num_valid=num_valid, seed=data_seed)
